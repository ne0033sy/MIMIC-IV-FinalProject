...
def load_config(config_path="config.yaml"):
    ...
    default_config = {
        'paths': {
            'raw_data_dir': './mimic-iv-2.2',  # Local MIMIC-IV data directory
...
class PathManager:
    ...
    def _create_directories(self):
        ...
        for path_key, path_value in self.paths.items():
            if path_key != 'raw_data_dir':  # Don't create raw_data_dir as it should already exist
    ...
    def get_raw_file_path(self, table_name):
        """Get path to raw MIMIC-IV files"""
        # Map table names to their directory structure in MIMIC-IV
        table_map = {
        ...
        if table_name in table_map:
            return os.path.join(self.paths['raw_data_dir'], table_map[table_name])
        else:
...
class MIMICPipeline:
    ...
    def run_full_pipeline(self):
        ...
        try:
            self.logger.info("=== Starting Memory-Optimized MIMIC-IV Local Pipeline ===")
            initial_memory = self.memory_manager.get_memory_usage()
            
            # Step 1: Select cohort
            self.logger.info("=== Step 1: Selecting cohort ===")
            cohort_df = self.cohort_selector.select_cohort()
            
            # Step 2: Process vital signs
            self.logger.info("=== Step 2: Processing vital signs ===")
            cohort_path = self.path_manager.get_path('local_processed_dir', 'cohort.csv')
            self._process_all_vitals(cohort_path)
            
            # Step 3: Create hourly bins
            self.logger.info("=== Step 3: Creating hourly bins ===")
            processed_vitals = self.hourly_binner.create_hourly_bins_for_vitals(cohort_path)
            
            # Step 4: Merge all vitals
            self.logger.info("=== Step 4: Merging all vitals ===")
            ...
            self.logger.info("=== Step 5: Creating sliding windows ===")
...
def main():
    ...
    try:
        ...
        if not os.path.exists("config.yaml"):
            ...
            print("Make sure to set 'raw_data_dir' to point to your local MIMIC-IV data directory.")
            return
        ...
        if not os.path.exists(raw_data_dir):
            ...
            print("Please make sure MIMIC-IV data is available and update the 'raw_data_dir' path in config.yaml")
            return
        ...
        print(f"Processed data saved in: {config['paths']['output_dir']}")
...
def run_pipeline_step(step_name, config_path="config.yaml"):
    ...
    if step_name == "cohort":
        ...
        sequences_array = np.load(sequences_path, allow_pickle=True)  # allow_pickle=True 추가
        ...
        print("Available steps: cohort, vitals, bins, merge, windows, finalize")   
