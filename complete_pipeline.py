...
# ===========================================================
# Data Quality Checker (Memory Optimized)
# ===========================================================
class DataQualityChecker:
    ...
    def check_temporal_coverage(self, df, time_col='charttime', stay_id_col='stay_id', chunk_size=1000):
        """Memory-optimized temporal coverage check"""
        coverage_stats = []
        unique_stays = df[stay_id_col].unique()
        
        # Process stays in chunks
        for i in range(0, len(unique_stays), chunk_size):
            stay_chunk = unique_stays[i:i + chunk_size]
            chunk_data = df[df[stay_id_col].isin(stay_chunk)].copy()
            chunk_data[time_col] = pd.to_datetime(chunk_data[time_col])
            
            for stay_id in stay_chunk:
                stay_data = chunk_data[chunk_data[stay_id_col] == stay_id]
...
# ===========================================================
# Enhanced AWS S3 Utilities (Unchanged)
# ===========================================================
class S3Manager:
    def __init__(self, logger):
        self.logger = logger
...
# ===========================================================
# Memory-Optimized Vital Signs Processor
# ===========================================================
class VitalSignsProcessor:
    ...
    def process_respiratory_rate(self, cohort_path, chartevents_path, procedureevents_path):
        """Memory-optimized RR processing with predefined schema"""
        try:
            self.logger.info("Processing respiratory rate with memory optimization...")
            ...
            rr_df = spark.read.csv(chartevents_path, header=True, schema=self.schemas.get_chartevents_schema()) \
            ...
            # Apply value range filters early to reduce data volume
...
# ===========================================================
# Memory-Optimized Hourly Binning
# ===========================================================
class HourlyBinner:
    def __init__(self, config, path_manager, logger):
        self.config = config
...
class DataMerger:
    ...
    def merge_all_vitals(self):
        ...
        try:
            ...
            # Chunked coverage analysis
            coverage_stats = []
            chunk_size = self.config['processing']['chunk_size'] // 5
            unique_stays = merged['stay_id'].unique()
            
            for i in range(0, len(unique_stays), chunk_size):
                stay_chunk = unique_stays[i:i + chunk_size]
                chunk_data = merged[merged['stay_id'].isin(stay_chunk)]
                
                chunk_stats = chunk_data.groupby('stay_id').agg({
                    'hour_from_intime': ['min', 'max', 'count'],
                    'subject_id': 'first'
                }).reset_index()
                
                chunk_stats.columns = ['stay_id', 'min_hour', 'max_hour', 'total_hours', 'subject_id']
                chunk_stats['hour_range'] = chunk_stats['max_hour'] - chunk_stats['min_hour'] + 1
                chunk_stats['coverage_ratio'] = chunk_stats['total_hours'] / chunk_stats['hour_range']
                
                coverage_stats.append(chunk_stats)
            ...
            self.logger.info(f"  Mean coverage ratio: {coverage_df['coverage_ratio'].mean():.3f}")
            self.logger.info(f"  Median total hours: {coverage_df['total_hours'].median():.1f}")
            ...
            # Chunked missing analysis
            feature_cols = [col for col in merged.columns if col not in merge_keys]
            missing_analysis = self.quality_checker.check_missing_rate(merged, feature_cols, chunk_size=1000)
...
# ===========================================================
# Memory-Optimized Sliding Window Creator
# ===========================================================  
class SlidingWindowCreator:
    ...
    def _determine_window_label(self, deathtime, obs_start_time, obs_end_time, 
        ...
        """Determine window label (unchanged)"""
        remove_window = False
        new_label = 0
...
# ===========================================================
# Memory-Optimized Pipeline Orchestrator
# ===========================================================
class MIMICPipeline:
    ...
    def run_full_pipeline(self):
        """Execute the memory-optimized preprocessing pipeline"""
        try:
            self.logger.info("=== Starting Memory-Optimized MIMIC-IV Pipeline ===")
            initial_memory = self.memory_manager.get_memory_usage()
    ...
    def _download_raw_data(self):
        """Download raw data from S3 (unchanged)"""
        raw_files = {
    ...
    def _process_all_vitals(self, cohort_path, raw_file_paths):
        """Process all vital signs (unchanged)"""
    ...
    def _upload_final_results(self):
        """Upload final results to S3 (unchanged)"""
...
class DataCleaner:
    ...
    def clean_and_process_data(self):
        ...
        try:
            self.logger.info("Starting memory-optimized data cleaning...")
            initial_memory = self.memory_manager.get_memory_usage()
            ...
            # Load original data with dtype optimization
            dtype_dict = {
                'subject_id': 'int64',
                'hadm_id': 'int64',
                'stay_id': 'int64',
            ...
            original_data = pd.read_csv(data_path, dtype=dtype_dict)
            ...
            del original_data, cohort
            gc.collect()
            
            # Recreate sequences with chunked processing
            obs_window_hours = self.config['processing']['obs_window_hours']
            new_sequences = []
            
            chunk_size = self.config['processing']['chunk_size'] // 50
            ...
            for i in range(0, total_windows, chunk_size):
                chunk_end = min(i + chunk_size, total_windows)
                chunk_metadata = metadata.iloc[i:chunk_end]
                ...
                for _, row in chunk_metadata.iterrows():
                    ...
                    if subject_id not in processed_grouped.groups:
                        ...
                        chunk_sequences.append(empty_sequence)
                        continue
                    ...
                    chunk_sequences.append(np.array(sequence_data))
                
                new_sequences.extend(chunk_sequences)
                
                # Memory cleanup
                if i % (chunk_size * 5) == 0:
                    gc.collect()
                    current_memory = self.memory_manager.get_memory_usage()
                    self.logger.info(f"Processed {chunk_end}/{total_windows} windows, Memory: {current_memory:.2f}GB")
            ...
            processed_data_with_cohort.to_csv(os.path.join(output_dir, 'processed_hourly_data.csv'), index=False)
            ...
            final_memory = self.memory_manager.get_memory_usage()
            self.logger.info(f"Data cleaning completed. Memory: {initial_memory:.2f}GB -> {final_memory:.2f}GB")
            self.logger.info(f"Final sequence shape: {new_sequences_array.shape}")
...
# ===========================================================
# Configuration File Generator (Unchanged)
# ===========================================================
